...
[fs,history] = sequentialfs(@class_SVR_rbf,x_train,y_train,'cv',cvp,'direction','backward');
...
bestcv=0;
    for c=0.1:1:500
        for g=0.0001:0.0001:0.2
            cmd=['-s 3 -t 2 -v 5 -c ', num2str(c),'-g ', num2str(g)];
...
cmd=['-s 3 -t 2 -c ', num2str(bestc),'-g ',num2str(g)];
